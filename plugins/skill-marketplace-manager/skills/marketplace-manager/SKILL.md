...
|---|---|---|---|
| Publish | "publish X", "ship X", "release X", "push X to marketplace", "I have finished building X" | `/marketplace:publish` | artifact name |
| Validate | "validate X", "check X is valid", "is X ready to publish?", "run validation on X" | `/marketplace:validate` | artifact name |
| New skill | "new skill called X", "create a skill X", "scaffold a skill named X" | `/marketplace:new-skill` | skill name |
| New plugin | "new plugin called X", "create a plugin X", "scaffold a plugin named X" | `/marketplace:new-plugin` | plugin name |
| Status | "what is live in my marketplace?", "status", "what needs publishing?", "show me the marketplace", "check the status", "what skills have not been published yet?" | `/marketplace:status` | none |
...
1. Run the `/marketplace:status` command first to show what exists and what is out of date
