...
Complete template content, ecosystem paths, guard check logic, directory creation order, confirmation output format, and silent validation checks for the `/marketplace:new-plugin` command. This is the single source of truth for all plugin scaffold details.
...
```bash
PLUGINS_DIR="/Users/mikey-east/Documents/claude-code-development/resources/utilities/claude-ecosystem/claude-plugins"
```
...
NAME="$ARGUMENTS"
if [ -z "$NAME" ]; then
  echo "Error: No name provided. Usage: /marketplace:new-plugin [name]"
  # STOP — do not proceed
fi
```
...
On failure output:
```
Error: No name provided. Usage: /marketplace:new-plugin [name]
```
...
if [ -d "$PLUGINS_DIR/$NAME" ]; then
  echo "Error: Plugin '$NAME' already exists at claude-plugins/$NAME/"
  # STOP — do not proceed
fi
```
...
On failure output:
```
Error: Plugin 'NAME' already exists at claude-plugins/NAME/
```
...
Path: `claude-plugins/NAME/.claude-plugin/plugin.json`
...
Path: `claude-plugins/NAME/README.md`
...
Path: `claude-plugins/NAME/CHANGELOG.md`
...
Step 1 implicitly creates the container directory `claude-plugins/NAME/` — no separate mkdir needed for the container.
...
Run each check with Bash. If ALL pass, output the confirmation format above. If ANY fail, output the full failure report using the same format as `/marketplace:validate`.

### Check 1 — Folder exists (Category 1)

```bash
[ -d "$PLUGINS_DIR/$NAME" ] && echo "PASS" || echo "FAIL: Folder missing at claude-plugins/$NAME/"
```
