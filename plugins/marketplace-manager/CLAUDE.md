...
- Example: `/marketplace-manager:validate recipe-optimiser` → `$ARGUMENTS = recipe-optimiser`
...
|------|---------|---------|
| Local plugin loading | `claude --plugin-dir ./marketplace-manager` or `cc --plugin-dir ./marketplace-manager` | Load plugin without installing; use during development |
...
**Directives:**
- **Communication:** Default to concise responses and avoid unnecessary preamble. When the developer gives a short approval or command, act on it immediately without requesting confirmation. Reserve longer explanations for when the developer explicitly asks for walkthroughs or context.
- **Decisions:** When presenting options, include a clear recommendation with brief reasoning. Lead with the recommended choice rather than neutral lists. When the developer delegates, make a decisive recommendation and proceed unless they redirect.
- **Explanations:** Provide step-by-step walkthroughs when introducing new concepts, tools, or workflows. Use plain accessible language and define technical terms on first use. When completing a task, explain what was done and why in before/after format.
- **Debugging:** When debugging, explain the root cause before proposing fixes. Do not offer workarounds without first diagnosing the underlying issue. When the developer pastes an error, take the lead on investigation.
- **UX Philosophy:** Focus on functionality, pipeline correctness, and CLI usability. Do not introduce frontend or UI concerns unless the developer raises them.
- **Vendor Choices:** Recommend the best option directly with a brief rationale rather than presenting multiple alternatives. Only present alternatives when the choice has significant long-term implications.
- **Frustrations:** Follow established patterns and prior instructions carefully. Do not ask the developer to perform steps that Claude can execute directly. When instructions are given, follow them precisely.
- **Learning:** Teach through guided walkthroughs — explain concepts step by step using plain language and everyday analogies before technical detail. Proactively offer brief explanations rather than assuming familiarity.
<!-- GSD:profile-end -->
