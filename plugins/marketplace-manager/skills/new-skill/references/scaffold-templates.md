...
All template content, ecosystem paths, guard check logic, directory creation order, confirmation output format, and silent validation checks for the `/marketplace-manager:new-skill` command.
...
If $ARGUMENTS is empty or whitespace only:
  Output: Error: No name provided. Usage: /marketplace-manager:new-skill [name]
...
```bash
SKILLS_DIR="/Users/michaeleast/Documents/claude-code-development/resources/utilities/claude-ecosystem/claude-skills"
[ -d "$SKILLS_DIR/$NAME" ] && {
...
On failure output exactly:
`Error: Skill 'NAME' already exists at skills/NAME/`
...
### File 1 — `skills/NAME/claude-code-skill/SKILL.md`
...
### File 3 — `skills/NAME/CHANGELOG.md`
...
### Directory 1 — `skills/NAME/claude-desktop-skill/`
...
1. `mkdir -p skills/NAME/claude-code-skill/`
2. `mkdir -p skills/NAME/claude-desktop-skill/`
3. Write `skills/NAME/claude-code-skill/SKILL.md` — substitute NAME in both the `name:` field and the `# Heading` line
4. Write `skills/NAME/README.md` — substitute NAME in the title heading
5. Write `skills/NAME/CHANGELOG.md` — no substitution needed
...
Run all 12 checks. If all pass, output the `✔ validate: NAME — all checks passed` line. If any fail, output the full failure report using the same format as `/marketplace-manager:validate`.
...
*This file is the complete template and logic reference for the `/marketplace-manager:new-skill` command. SKILL.md loads this file explicitly via `Read: ${CLAUDE_SKILL_DIR}/references/scaffold-templates.md`.*
